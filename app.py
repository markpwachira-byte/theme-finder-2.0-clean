...
def index():
    ...
    if request.method == "POST":
        ...
        if not file:
            return render_template("index.html", results=["Please upload a file."])
        
        filename = file.filename.lower()
        text = ""

        # Processing PDF/TXT logic
        ...
        if theme and theme in theme_keywords:
            ...
            scored = sorted([(p, sum(1 for kw in keywords if kw.lower() in p.lower())) 
                            for p in paragraphs], key=lambda x: x[1], reverse=True)
